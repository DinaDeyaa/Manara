...
def get_related_previous_subtopics(
    ...
    if target_concepts_collection is None:
        ...
        print("DEBUG target concepts collection missing. Using fallback:", len(fallback))
        return fallback
...
def build_fallback_context_from_item(item: dict) -> str:
    return (
        f"=== FALLBACK CONCEPT ===\n"
        f"Source course: {item.get('source_course', '')}\n"
        f"Source file: {item.get('source_relative_path', '')}\n"
        f"Source chapter: {item.get('source_chapter', '')}\n"
        f"Source topic: {item.get('source_topic_name', '')}\n"
        f"Source subtopic: {item.get('source_subtopic_name', '')}\n\n"
        f"Target course: {item.get('target_course', '')}\n"
        f"Target topic: {item.get('target_topic_name', '')}\n\n"
        f"{item.get('target_concept_text', '')}"
    ).strip()
...
def build_diagnostic_exam_questions(
    ...
    for item, difficulty in zip(related_items, difficulties):
        ...
        if not context_text.strip():
            context_text = build_fallback_context_from_item(item)

        if not context_text.strip():
            print(f"Skipped '{item['source_subtopic_name']}': no context available")
            continue

        banned_questions = [q["question"] for q in question_rows]
        banned_concepts = [extract_question_concept(q["question"]) for q in question_rows]
    ...
    if not question_rows:
        raise ValueError(
            "No diagnostic exam questions could be generated. "
            "Check that the student's completed courses have chapter_concepts.json files "
            "and that the selected target course has retrievable material or fallback concepts."
        )
